...
public class Water : MonoBehaviour
{
	...
	void Generate(int width, float top, float bot)
	{
		...
		Vector2[] nxy = new Vector2[amountData];
		Vector2[] nz = new Vector2[amountData];
		for (int i = 0; i < amountData; ++i)
		{
			Vector3 n = normals[i];
			nxy[i] = new Vector2(n.x, n.y);
			nz[i] = new Vector2(n.z, 0f);
		}
    ...
    void Update()
	{
		...
		float s = 0.5f * (Mathf.Sin(time * timeScale) + 1.0f);
